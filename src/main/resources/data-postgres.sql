...
begin
	...
	loop
		...
		loop
		    for s in select id from t_products
		    loop
			    insert into t_contracts(user_id, product_id, start_date_time, end_date_time)
			    values(r.id, s.id, d.day, d.day + interval '1 year');
		    end loop;
        end loop;
	end loop;
END$$;